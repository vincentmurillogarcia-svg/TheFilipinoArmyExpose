...
async function read(_, key, fallback) {
  try {
...
async function write(_, key, val) {
...
async function addLog(_, action, detail) {
  try {
    const log = await read(null, "activityLog", []);
    log.unshift({ action, detail, timestamp: new Date().toISOString() });
    if (log.length > 500) log.length = 500;
    await write(null, "activityLog", log);
  } catch {}
}
  ...
  try {
    ...
    if (req.method === "GET" && route === "data") {
      const [posts, comments, announcements, categories, settings, reactions] = await Promise.all([
        read(null, "posts",         []),
        read(null, "comments",      []),
        read(null, "announcements", []),
        read(null, "categories",    null),
        read(null, "settings",      {}),
        read(null, "reactions",     {}),
      ]);
    ...
    if (req.method === "GET" && route.startsWith("profile/")) {
      ...
      const users = await read(null, "users", {});
      const user  = users[uname];
      if (!user) return err("User not found", 404);
      const posts = await read(null, "posts", []);
    ...
    if (route === "auth") {
      ...
      if (action === "register") {
        ...
        const users   = await read(null, "users",        {});
        const pending = await read(null, "pendingUsers", []);
        ...
        await write(null, "pendingUsers", pending);
        await addLog(null, "REGISTER_PENDING", `New pending registration: ${uname}`);
      ...
      if (action === "login") {
        ...
        if (changed) await write(null, "users", users);

        await addLog(null, "LOGIN", uname);
      ...
      if (action === "updateProfile") {
        ...
        await write(null, "users", users);
        await addLog(null, "UPDATE_PROFILE", uname);
      ...
      if (action === "changePassword") {
        ...
        user.passwordHash = hashPw(newPassword);
        await write(null, "users", users);
        await addLog(null, "PASSWORD_CHANGE", uname);
      ...
      if (action === "changeUsername") {
        ...
        await write(null, "posts", posts);

        // Migrate comments
        const comments = await read(null, "comments", []);
        comments.forEach(c => { if (c.authorUsername === uname) c.authorUsername = newUsername; });
        await write(null, "comments", comments);

        await addLog(null, "CHANGE_USERNAME", `${uname} → ${newUsername}`);
    ...
    if (route === "posts") {
      ...
      posts.unshift(post);
      await write(null, "posts", posts);
      await addLog(null, "NEW_POST", post.title);
    ...
    if (route === "reactions") {
      ...
      const reactions = await read(null, "reactions", {});
      ...
      await write(null, "reactions", reactions);
    ...
    // ══════════════════════════════════════════════════════
    if (route === "flag") {
      await addLog(null, "FLAG", body.id || "unknown");
      return ok();
    }
    ...
    if (route === "tips") {
      ...
      const tips = await read(null, "tips", []);
      tips.unshift({
        id:          genId(),
        title:       body.title.trim().slice(0, 200),
      ...
      await write(null, "tips", tips);
      await addLog(null, "TIP_RECEIVED", body.title);
      return ok();
    }
    ...
    if (route === "admin") {
      ...
      const settings = await read(null, "settings", {});
      ...
      switch (action) {
        ...
        case "status": {
          ...
          await write(null, "posts", posts);
          await addLog(null, "STATUS", `"${post.title}" → ${data.status}`);
          break;
        }
        ...
        case "urgency": {
          ...
          await write(null, "posts", posts);
          await addLog(null, "URGENCY", `"${post.title}" → ${data.urgency}`);
          break;
        }
        ...
        case "pin": case "unpin": {
          ...
          await write(null, "posts", posts);
          await addLog(null, action.toUpperCase(), post.title);
          break;
        }
        ...
        case "editPost": {
          ...
          await write(null, "posts", posts);
          await addLog(null, "EDIT_POST", post.title);
          break;
        }
        ...
        case "bulkStatus": {
          ...
          await write(null, "posts", posts);
          await addLog(null, "BULK_STATUS", `${ids.size} posts → ${data.status}`);
          break;
        }
        case "bulkDelete": {
          const [posts, comments] = await Promise.all([read(null,"posts",[]), read(null,"comments",[])]);
          const ids = new Set(data.ids || []);
          await Promise.all([
            write(null, "posts",    posts.filter(p => !ids.has(p.id))),
            write(null, "comments", comments.filter(c => !ids.has(c.postId))),
          ]);
          await addLog(null, "BULK_DELETE", `${ids.size} posts deleted`);
          break;
        }
        case "unpinAll": {
          const posts = await read(null, "posts", []);
          ...
          await write(null, "posts", posts);
          await addLog(null, "UNPIN_ALL", "All posts unpinned");
          break;
        }

        // ── Announce ──────────────────────────────────────
        case "announce": {
          const anns = await read(null, "announcements", []);
          anns.unshift({ title: data.title, content: data.content, timestamp: new Date().toISOString() });
          await write(null, "announcements", anns);
          await addLog(null, "ANNOUNCE", data.title);
          break;
        }
        case "clearAnn": {
          await write(null, "announcements", []);
          await addLog(null, "CLEAR_ANN", "All announcements cleared");
          break;
        }
        case "dismissAnn": {
          const anns = await read(null, "announcements", []);
        ...
        case "claimFull": case "claimCo": {
          ...
          await write(null, "posts", posts);
          await addLog(null, "CLAIM_POST", `"${p.title}" → ${claimerName} (${action})`);
          break;
        }
        ...
        case "deleteComment": {
          const comments = await read(null, "comments", []);
          await write(null, "comments", comments.filter(c => c.id !== data.commentId));
          await addLog(null, "DELETE_COMMENT", `Comment: ${data.commentId}`);
          break;
        }
        ...
        case "delete": {
          ...
          await Promise.all([
            write(null, "posts",    posts.filter(p => p.id !== data.id)),
            write(null, "comments", comments.filter(c => c.postId !== data.id)),
          ]);
          await addLog(null, "DELETE_POST", title);
          break;
        }
        ...
        case "approveUser": {
          ...
          await Promise.all([write(null, "users", users), write(null, "pendingUsers", pending)]);
          await addLog(null, "APPROVE_USER", `${pu.username} as ${data.role || "citizen"}`);
          break;
        }
        case "rejectUser": {
          const pending = await read(null, "pendingUsers", []);
          ...
          if (idx >= 0) {
            ...
            pending.splice(idx, 1);
            await write(null, "pendingUsers", pending);
            await addLog(null, "REJECT_USER", name);
          }
          break;
        }
        case "getAllUsers": {
        ...
        case "setRole": {
          ...
          user.role = data.role;
          await write(null, "users", users);
          await addLog(null, "SET_ROLE", `${data.username} → ${data.role}`);
          break;
        }
        ...
        case "setDisplayName": {
          ...
          await write(null, "users", users);
          await addLog(null, "SET_DISPLAYNAME", `${data.username} → ${data.displayName}`);
          break;
        }
        ...
        case "postTip": {
          ...
          posts.unshift(post);
          tip.status = "posted";
          await Promise.all([write(null, "posts", posts), write(null, "tips", tips)]);
          await addLog(null, "TIP_POSTED", tip.title);
          return ok({ postId: post.id });
        }
        case "dismissTip": {
          const tips = await read(null, "tips", []);
          const tip  = tips.find(t => t.id === data.tipId);
          if (tip) { tip.status = "dismissed"; await write(null, "tips", tips); }
          await addLog(null, "TIP_DISMISSED", tip?.title || data.tipId);
          break;
        }
        ...
        case "passkey": {
          ...
          await write(null, "settings", settings);
          await addLog(null, "PASSKEY_CHANGE", "Developer passkey updated");
          break;
        }
        ...
        case "setReactions": {
          ...
          settings.customReactions = data.reactions.map(r => String(r).slice(0, 8)).filter(Boolean);
          await write(null, "settings", settings);
          await addLog(null, "SET_REACTIONS", settings.customReactions.join(" "));
          break;
        }
        ...
        case "categories": {
          ...
          await write(null, "categories", data.categories);
          break;
        }
        ...
        case "maintenance": {
          ...
          await write(null, "settings", settings);
          await addLog(null, "MAINTENANCE", data.enabled ? "Maintenance mode ON" : "Maintenance mode OFF");
          break;
        }
        ...
        case "exportData": {
          const [posts, comments, users] = await Promise.all([
            read(null, "posts", []), read(null, "comments", []), read(null, "users", {}),
          ]);
          const safeUsers = Object.fromEntries(
            Object.entries(users).map(([k, v]) => [k, safeUser(v)])
          );
          await addLog(null, "EXPORT", "Data exported by admin");

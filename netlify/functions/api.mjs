...
export default async (req) => {
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════
    // POST /api/staff  — staff moderation actions (no passkey)
    // Verifies caller is an approved staff or developer user.
    // Staff can: verify/review/unverify posts, approve/reject
    //            pending users (as citizen/reporter only),
    //            delete comments.
    // ══════════════════════════════════════════════════════
    if (route === "staff") {
      ...
      // Verify caller is staff or developer
      const { data: caller } = await supabase
        .from("users").select("role, approved").eq("username", username.toLowerCase()).single();
      ...
      switch (action) {
        ...
        case "status": {
          const { data: post } = await supabase
            .from("posts").select("status_history, title").eq("id", sdata.id).single();
          if (!post) return err("Post not found");
          const history = [...(post.status_history || []), { status: sdata.status, timestamp: new Date().toISOString() }];
          await supabase.from("posts")
            .update({ status: sdata.status, status_history: history }).eq("id", sdata.id);
          await addLog("STAFF_STATUS", `"${post.title}" → ${sdata.status} by ${username}`);
          break;
        }
        ...
        case "verify": {
          ...
          await supabase.from("posts").update({ status: "verified", status_history: history }).eq("id", sdata.id);
          await addLog("STAFF_VERIFY", `"${post.title}" by ${username}`);
          break;
        }
        ...
        case "review": {
          ...
          await supabase.from("posts").update({ status: "reviewing", status_history: history }).eq("id", sdata.id);
          await addLog("STAFF_REVIEW", `"${post.title}" by ${username}`);
          break;
        }
        ...
        case "unverify": {
          ...
          await supabase.from("posts").update({ status: "unverified", status_history: history }).eq("id", sdata.id);
          await addLog("STAFF_UNVERIFY", `"${post.title}" by ${username}`);
          break;
        }

        case "deleteComment": {
          await supabase.from("comments").delete().eq("id", sdata.commentId);
          await addLog("STAFF_DEL_CMT", `Comment: ${sdata.commentId} by ${username}`);
          break;
        }
        ...
        case "approveUser": {
          ...
          await supabase.from("pending_users").delete().eq("username", sdata.username);
          await addLog("STAFF_APPROVE", `${pu.username} as ${assignRole} by ${username}`);
          break;
        }

        case "rejectUser": {
          await supabase.from("pending_users").delete().eq("username", sdata.username);
          await addLog("STAFF_REJECT", `${sdata.username} by ${username}`);
          break;
        }
